...
import {
    ...
    DollarSign, Receipt, CreditCard, Wallet, Banknote, ShieldCheck, Database,
    Upload
} from 'lucide-react';
...
export default function ContasReceber() {
    ...
    const handleImportXLSX = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        reader.onload = (evt) => {
            ...
            alert('Recebíveis detectados! Iniciando importação em massa para o banco...');
        };
    ...
    const stats = useMemo(() => {
        ...
        const toReceive = allInst.filter(i => i.status === 'PENDING').reduce((acc, i) => acc + i.amount, 0);
        const overdue = allInst.filter(i => i.status === 'OVERDUE').reduce((acc, i) => acc + i.amount, 0);
        const receivedToday = allInst.filter(i => i.status === 'PAID' && i.payment_date?.startsWith(new Date().toISOString().split('T')[0])).reduce((acc, i) => acc + i.amount, 0);

        return { toReceive, overdue, receivedToday };
    }, [entries]);
        ...
        <div className="reveal space-y-16 w-full pb-20">
            ...
            <header className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-12 bg-[var(--bg-card)]/40 p-12 rounded-2xl border border-white/5 shadow-2xl">
                <div className="space-y-6">
                    <div className="flex items-center gap-3">
                        ...
                        <h2 className="text-caption text-[var(--primary)] tracking-[0.4em] text-sm">Gestão de Recebíveis • Operacional</h2>
                    </div>
                    <h1 className="h1">Contas a <span className="text-[var(--primary)]">Receber</span></h1>
                    <p className="text-lg font-medium text-[var(--text-muted)] max-w-2xl leading-relaxed">
                        Interface de alta densidade para controle de mensalidades, matrículas e aportes institucionais.
                        Gere lançamentos manuais para <strong className="text-white">Conciliação Inteligente</strong>.
                    </p>
                </div>
                <div className="flex items-center gap-12">
                    <button
                        onClick={() => document.getElementById('xlsx-import-cr')?.click()}
                    ...
                    >
                        ...
                        <span className="text-[12px] font-black uppercase tracking-[0.3em] text-[#F43F5E]">IMPORTAR RECEBÍVEIS</span>
                        <input
                            id="xlsx-import-cr"
                            type="file"
                            style={{ display: 'none' }}
                    ...
                    <div className="flex gap-6">
                        <button className="btn btn-ghost !px-8">
                            <FileDown size={20} className="text-[var(--accent-azure)]" />
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
                <div className="card !p-10 border-l-4 border-l-[var(--accent-azure)] bg-gradient-to-br from-[var(--bg-card)] to-[#1D222B]">
                    <div className="flex justify-between items-start mb-6">
                        <p className="text-[10px] font-black text-[var(--text-disabled)] uppercase tracking-widest">A Receber (Projetado)</p>
                        <Clock size={16} className="text-[var(--accent-azure)] opacity-50" />
                    </div>
                    <h3 className="text-4xl font-black text-white tracking-tighter">R$ {stats.toReceive.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</h3>
                    <div className="flex items-center gap-2 mt-6 text-[var(--accent-azure)]">
                        <ArrowUpRight size={14} />
                        <span className="text-[10px] font-black uppercase">Expectativa de Fluxo 30D</span>
                    </div>
                </div>

                <div className="card !p-10 border-l-4 border-l-[var(--danger)] bg-gradient-to-br from-[var(--bg-card)] to-[#1D222B]">
                    <div className="flex justify-between items-start mb-6">
                        <p className="text-[10px] font-black text-[var(--text-disabled)] uppercase tracking-widest">Inadimplência / Atrasos</p>
                    ...
                    <div className="flex items-center gap-2 mt-6 text-[var(--danger)]">
                        <ShieldCheck size={14} className="text-[var(--danger)] opacity-60" />
                        <span className="text-[10px] font-black uppercase">{entries.filter(e => e.status === 'OVERDUE').length} registros críticos</span>
                    </div>
                </div>

                <div className="card !p-10 border-l-4 border-l-[var(--primary)] bg-gradient-to-br from-[var(--bg-card)] to-[#1D222B]">
                    <div className="flex justify-between items-start mb-6">
                        <p className="text-[10px] font-black text-[var(--text-disabled)] uppercase tracking-widest">Liquidação Diária</p>
                        <CheckCircle2 size={16} className="text-[var(--primary)] opacity-50" />
                    </div>
                    <h3 className="text-4xl font-black text-white tracking-tighter">R$ {stats.receivedToday.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</h3>
                    <div className="flex items-center gap-2 mt-6 text-[var(--primary)]">
                        <Banknote size={14} />
                        <span className="text-[10px] font-black uppercase">Recebido e Conciliado</span>
                    </div>
                </div>
            </div>
            ...
            <div className="card !p-8 bg-[var(--bg-card)]/40 border-white/5 flex flex-col xl:flex-row items-center gap-8">
                <div className="flex-1 flex items-center gap-5 bg-[var(--bg-input)] border border-white/5 rounded-2xl px-8 py-5 w-full">
                    ...
                    <input
                        placeholder="Buscar por cliente, documento, rubrica ou ID de transação..."
            ...
            <div className="card !p-0 overflow-hidden border-white/5 bg-[var(--bg-card)]/60 backdrop-blur-3xl shadow-2xl">
                <table className="w-full text-left">
                    <thead className="bg-[#151921] border-b border-white/5 text-[10px] uppercase font-black text-white/30 tracking-[0.25em]">
                        <tr>
                            <th className="p-10">ENTIDADE / DESCRIÇÃO</th>
                            <th className="p-10">VENCIMENTO</th>
                            <th className="p-10">CATEGORIA & UNIDADE</th>
                            <th className="p-10 text-right">VALOR BRUTO</th>
                            <th className="p-10 text-center">STATUS FLUXO</th>
                            ...
                            <tr>
                                <td colSpan={6} className="p-32 text-center">
                                    <div className="flex flex-col items-center gap-6 opacity-20">
                                        <Database size={48} />
                                        <p className="text-sm font-black uppercase tracking-[0.4em]">Nenhum registro encontrado</p>
                                    </div>
                                </td>
                            </tr>
                        ) : (
                                ...
                                <tr key={entry.id} className="hover:bg-white/[0.02] transition-all group">
                                    <td className="p-10">
                                        <div className="flex items-center gap-6">
                                            <div className="w-14 h-14 rounded-2xl bg-[var(--primary)]/5 border border-[var(--primary)]/10 flex items-center justify-center shadow-inner">
                                                <User size={22} className="text-[var(--primary)]" />
                                            </div>
                                            <div>
                                                <p className="text-base font-black text-white tracking-tight truncate max-w-[300px]">
                                                    {entry.entities?.name || 'Cliente Geral'}
                                                </p>
                                    ...
                                    <td className="p-10">
                                        <div className="flex justify-center gap-4">
                                            <button className="btn !p-4 bg-white/5 border border-white/10 hover:bg-[var(--primary)] hover:border-[var(--primary)] transition-all">
